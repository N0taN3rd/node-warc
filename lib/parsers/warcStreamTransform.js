...
/**
 * Transforms a WARC file ReadStream into its individual {@link WARCRecord}s
 * @extends {Transform}
 * @example
    ...
    switch (this.parsingState) {
      case parsingStates.header:
        if (!isSep) {
          ...
          this.wContentLength = Number(this.wHeader.get('Content-Length'))
          this.wType = this.wHeader.get('WARC-Type')
        }
        break
