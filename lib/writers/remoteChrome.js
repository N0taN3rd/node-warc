...
const responseEncoder = require('../utils/responseEncoder')
    ...
    */
    if (this._noHTTP2) {
      ...
      if (!res) {
        ...
        if (!wasError) {
          if (res.encoding) {
            if (responseEncoder.canCompress(res.encoding)) {
              let compressed
              try {
                compressed = await responseEncoder.compress(resData, res.encoding)
              } catch (error) {
                wasError = true
              }
            ...
            if (res.encoding === 'gzip') {
              resData = await gzip(resData)
              responseHeaders = responseHeaders.replace(replaceContentLen, `Content-Length: ${Buffer.byteLength(resData, 'binary')}${CRLF}`)
            } else if (res.encoding === 'deflate') {
              resData = await deflate(resData)
              responseHeaders = responseHeaders.replace(replaceContentLen, `Content-Length: ${Buffer.byteLength(resData, 'binary')}${CRLF}`)
            } else {
              responseHeaders = responseHeaders.replace(noGZ, '')
              responseHeaders = responseHeaders.replace(replaceContentLen, `Content-Length: ${Buffer.byteLength(resData, 'utf8')}${CRLF}`)
        ...
        if (!wasError) {
          if (res.encoding) {
            if (res.encoding === 'gzip') {
              ...
              finalRequestHeaders = finalRequestHeaders.replace(noGZ, '')
              finalRequestHeaders = finalRequestHeaders.replace(replaceContentLen, `Content-Length: ${Buffer.byteLength(resData, 'utf8')}${CRLF}`)
            }
          }
        }
