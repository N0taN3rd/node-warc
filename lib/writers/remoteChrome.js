...
class RemoteChromeWARCGenerator extends WARCWriterBase {
  ...
  async generatePost (nreq, network) {
    ...
    if (!wasError) {
      ...
      responseHeaders = responseHeaders.replace(replaceContentLen, `Content-Length: ${resData.length}${CRLF}`)
    }
    // console.log(responseHeaders)
    // console.log(responseHeaders)
  ...
  async generateRedirectResponse (nreq, network) {
    ...
    if (nreq.res) {
      ...
      if (!wasError) {
        ...
        finalResponseHeaders = finalResponseHeaders.replace(replaceContentLen, `Content-Length: ${resData.length}${CRLF}`)
      }
      // console.log(finalResponseHeaders)
