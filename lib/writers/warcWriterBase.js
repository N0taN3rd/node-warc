...
function makeContentBuffer (string, maybeBuffer) {
  const strBuffer = Buffer.from(
    `${CRLF}${
      string[string.length - 1] === CRLF[1] && string[string.length - 2] === CRLF[0]
        ? string
        : string + CRLF
    }`,
    'utf8'
  ...
  if (maybeBuffer != null) {
    ...
    return Buffer.concat([strBuffer, breqData], strBuffer.length + breqData.length)
  }
  return strBuffer
}
...
class WARCWriterBase extends EventEmitter {
  ...
  writeWarcMetadataOutlinks (targetURI, outlinks) {
    let wmhc = Buffer.from(`${CRLF}${outlinks}`, 'utf8')
    let wmh = Buffer.from(
      warcMetadataHeader({
        targetURI,
        now: this._now,
  ...
  writeRequestRecord (targetURI, httpHeaderString, requestData) {
    ...
    const totalLength =
      reqWHeader.length + reqHeadContentBuffer.length + recordSeparatorBuffer.length
    return this.writeRecordBlock(
      Buffer.concat(
  ...
  writeResponseRecord (targetURI, httpHeaderString, responseData) {
    const resHeaderContentBuffer = makeContentBuffer(httpHeaderString, responseData)
    ...
    const totalLength =
      respWHeader.length + resHeaderContentBuffer.length + recordSeparatorBuffer.length
    return this.writeRecordBlock(
      Buffer.concat(
