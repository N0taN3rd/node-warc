...
/**
 * Default options that control how WARC writting is done
...
class WARCWriterBase extends EventEmitter {
  ...
  /**
   * Initialize the writer. The options object is optional and defaults to `appending = false` and `gzip = process.env.NODEWARC_WRITE_GZIPPED != null`.
  ...
  async writeRecordChunks (...recordParts) {
    for (const chunk of recordParts) {
