...
class WARCWriterBase extends EventEmitter {
  ...
  initWARC (warcPath, appending) {
    if (appending) {
      this._warcOutStream = fs.createWriteStream(warcPath, { flags: 'a', encoding: 'utf8' })
    } else {
  ...
  writeRequestRecord (targetURI, httpHeaderString, requestData) {
    ...
    const totalLength = reqWHeader.length + recordSeparatorBuffer.length + reqHeadContentBuffer.length
    return this.writeRecordBlock(
      Buffer.concat([reqWHeader, reqHeadContentBuffer, recordSeparatorBuffer], totalLength))
  }
  ...
  writeResponseRecord (targetURI, httpHeaderString, responseData) {
    ...
    'utf8')
    const totalLength = respWHeader.length + resHeaderContentBuffer.length +
      CRLFBuffer.length + recordSeparatorBuffer.length
    if (responseData !== null && responseData !== undefined) {
      ...
      return this.writeRecordBlock(Buffer.concat([
        ...
        responseDataBuffer,
        CRLFBuffer,
        recordSeparatorBuffer], totalLength + responseDataBuffer.length))
    } else {
      return this.writeRecordBlock(Buffer.concat([
        respWHeader,
        resHeaderContentBuffer,
        CRLFBuffer,
        recordSeparatorBuffer], totalLength))
    }
  }
  ...
  _doWrite (dataIter, resolve, reject) {
    ...
    if (!next.done) {
      if (!this._warcOutStream.write(next.value, 'utf8')) {
        this._warcOutStream.once('drain', this._doWrite.bind(this, dataIter, resolve, reject))
      } else {
