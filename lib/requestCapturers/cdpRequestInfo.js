...
class CDPRequestInfo {
  ...
  constructor () {
    ...
    /**
     * The URL of the request. May not contain the fragment
  ...
  _checkMethod () {
    if (!this.method) {
      ...
      if (this.requestHeaders) {
        const maybeMeth = this.requestHeaders[H2Method]
        if (maybeMeth) {
      ...
      if (this.responseHeaders) {
        const maybeMeth = this.responseHeaders[H2Method]
        if (maybeMeth) {
          this.method = maybeMeth
          good = true
        }
  ...
  _methProtoFromReqHeadText (requestHeadersText) {
    if (requestHeadersText) {
      const httpString = requestHeadersText.substr(
        0,
        requestHeadersText.indexOf(CRLF)
      )
      if (httpString) {
        const httpStringParts = httpString.split(SPACE)
        if (httpStringParts) {
