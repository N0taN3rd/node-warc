...
test('CapturedRequest should handle initialization with a request', t => {
  ...
  t.truthy(
    cr,
    'the newly created captured request using new one should not be undefined or null'
  )
  t.is(
    cr.requestId,
    req.requestId,
    'captured request id should be the same as the request object used to create it'
  )
  t.is(cr._reqs.size, 1, 'captured request _reqs map should be of size one')
  t.is(
    Array.from(cr.keys())[0],
    req.request.url,
    'captured request keys should return the url of the captured request'
  )
  t.is(
    cr.url(),
    req.request.url,
    'captured request url method should return the url of the captured request'
  )
})
...
test('CapturedRequest should handle initialization with a redirection', t => {
  ...
  t.deepEqual(
    Array.from(cr.keys()),
    [redir.redirectResponse.url, redir.request.url],
    'captured request keys should return the url of the captured request'
  )
  t.deepEqual(
    cr.url(),
...
test('CapturedRequest should add a new redirection if not already present', t => {
  ...
  const urlar = cr.url()
  t.true(
    kar.indexOf(redir[0].request.url) !== -1,
    'the original request url should be present'
  )
  t.true(
    kar.indexOf(redir[1].redirectResponse.url) !== -1,
    'the redirect url should be present'
  )
  t.true(
    kar.indexOf(newR.redirectResponse.url) !== -1,
    'the new redirect url should be present'
  )
  t.true(kar.indexOf(newR.request.url) !== -1, 'the new request url should be present')
  t.true(
    urlar.indexOf(redir[0].request.url) !== -1,
    'the original request url should be present'
  )
  t.true(
    urlar.indexOf(redir[1].redirectResponse.url) !== -1,
    'the redirect url should be present'
  )
  t.true(
    urlar.indexOf(newR.redirectResponse.url) !== -1,
    'the new redirect url should be present'
  )
...
test('CapturedRequest should add a new response if not already present', t => {
  ...
  t.true(
    kar.indexOf(req[1].response.url) !== -1,
    'the new response url should be present'
  )
  t.true(
    urlar.indexOf(redir[0].request.url) !== -1,
    'the original request url should be present'
  ...
  t.true(
    urlar.indexOf(req[1].response.url) !== -1,
    'the new response url should be present'
  )
})
...
test('CapturedRequest should add a new request if not already present', t => {
  ...
  t.true(
    urlar.indexOf(req[0].request.url) !== -1,
    'the new request  url should be present'
  )
  cr.addRequestInfo(req[0])
  t.is(
    cr._reqs.size,
    3,
    'captured request _reqs map should be of size 3 after adding a duplicate request'
  )
})
...
test('CapturedRequest iterator should iterate over the request info', t => {
  ...
  t.is(
    reqData.url,
    req[0].request.url,
    'the first request info from the iterator should be about the request'
  )
  const rerData = iter.next().value
  t.is(
    rerData.url,
    redir[0].request.url,
    'the second request info from the iterator should be about the redirect'
  )
  const rerData2 = iter.next().value
  t.is(
    rerData2.url,
    redir[2].response.url,
    'the last request info from the iterator should be about the redirect'
  )
